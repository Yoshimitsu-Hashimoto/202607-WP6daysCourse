...
> **メモ：** 静的HTMLには「農園・精製方法・風味」もありますが、時間等の都合上この時点で削除させていただきます🙏
...
■get_post_type_archive_link( 'menu' ) と /menu/の違いについて
> **`/menu/` と直接書くのとどう違う？**
> Chapter 6 では `href="/menu/"` と直接URLを書きました。あの書き方でも動きますが、実は `get_post_type_archive_link( 'menu' )` を使う方が推奨です。
> 直接書いたURLは「サイトがドメイン直下にある」前提なので、サブディレクトリ（例：`example.com/shop/`）にWordPressを設置した場合や、カスタム投稿タイプのスラッグを後から変更した場合にリンク切れになります。関数で生成しておけば、環境が変わっても常に正しいURLを返してくれます。
  ...
  "settings": {
    ...
    "typography": {
      ...
      "fontSizes": [
        ...
        { "slug": "section-title", "size": "clamp(2.2rem, 4.5vw, 3.2rem)", "name": "Section Title" }
      ]
    },
    "layout": {
      "contentSize": "900px",
...
`fontFamilies` で「ベース（ゴシック）」と「ディスプレイ（セリフ）」の2つだけを並べます。見出しに Cormorant を当てたいときはここから「セリフ」を選ぶ運用です。`fontSizes` は Small〜X-Large の4段階に加え、5つ目に **「Section Title」（`clamp(2.2rem, 4.5vw, 3.2rem)`）** を用意しています。これは OUR VALUES や ORIGINS のような**セクション見出し専用のサイズ**で、見出しをブロックスタイルにせず「フォント＝セリフ＋サイズ＝Section Title」をパネルから選んで整えるための布石です。
...
```
テーブルブロックを選択
→ サイドバー「スタイル」タブに「産地テーブル」が追加される
→ 選ぶと HTML に is-style-origin-table クラスが付く
→ CSS（9-6）でそのクラスにデザインを当てる
```
...
クラシックテーマでは `.data-table` や `.feature-item` のような**自分で付けたクラス**にスタイルを書いていました。しかしブロックエディターが出力する HTML には、それらのクラスは付きません。代わりに付くのが `is-style-origin-table` のような**ブロックスタイル由来のクラス**です。

```
クラシック時代：<table class="data-table">           → .data-table { ... }
ブロック時代：  <table class="...is-style-origin-table"> → .is-style-origin-table { ... }
```
...
見出しの装飾は**挿入後にタイポグラフィパネルで設定**します（このパターンはブロックスタイルではなく、フォントとサイズを選んで見た目を作る方式です）。

1. 見出し「OUR VALUES」を選択 → ツールバーで**中央寄せ**にし、「タイポグラフィ」で**フォント＝セリフ**、**サイズ＝Section Title** を選ぶ
2. 小見出し「私たちの3つの誓い」を選択 → **中央寄せ**にして**サイズ＝Small** を選ぶ

> 色は触りません。ダーク背景のグループ（`has-dark-inverse-color`）が文字を自動で白にします。
...
```
グループ（背景：クリーム）
├ 見出し（H2）「ORIGINS」（中央寄せ／フォント：セリフ・サイズ：Section Title）
├ 段落「取り扱い産地」（中央寄せ／サイズ：Small）
└ テーブル（スタイル：産地テーブル）
```

操作のポイント：

1. **見出し（H2）**「ORIGINS」を置き、ツールバーで**中央寄せ**にする。「タイポグラフィ」で**フォント＝セリフ**、**サイズ＝Section Title** を選ぶ
2. 小見出しの段落「取り扱い産地」を置き、**中央寄せ**にして**サイズ＝Small** を選ぶ（スタイルは選ばない）
...
「フォーム」タブのテキストエリアを**すべて削除**し、以下をそのまま貼り付けます。

```html
<div class="form-row">
  <label class="form-label">お名前 <span class="required">必須</span></label>
  [text* name class:form-input placeholder "山田 太郎"]
</div>

<div class="form-row">
  <label class="form-label">メールアドレス <span class="required">必須</span></label>
  [email* email class:form-input placeholder "example@mail.com"]
</div>

<div class="form-row">
  <label class="form-label">件名</label>
  [text subject class:form-input placeholder "お問い合わせ件名"]
</div>

<div class="form-row">
  <label class="form-label">メッセージ <span class="required">必須</span></label>
  [textarea* message class:form-textarea placeholder "ご質問・ご相談内容をご記入ください。"]
</div>

<p class="form-note">ご入力いただいた個人情報は、お問い合わせへの返信目的のみに使用し、第三者への提供は行いません。</p>

<div class="form-submit">
  [submit class:btn class:btn--dark "送信する"]
</div>
```

- `*`（アスタリスク）= 必須フィールド。お名前・メール・メッセージに付け、件名はなし
- `class:form-input` のようにタグ内にクラスを指定することで、静的HTMLの既存CSSをそのまま当てられる

「保存」を押した後、フォーム上部に表示されるショートコード（例：`[contact-form-7 id="1"]`）をコピーしておきます。

---

### 10-3. functions.php に CF7 の自動 br 無効化を追加する

CF7 のフォームテンプレートでは改行が自動的に `<br>` に変換されるため、ラベルと入力欄の間に不要な空白行が入ります。`functions.php` の末尾に以下を追記して無効化します。

```php
// CF7 フォームテンプレート内の自動 <br> 変換を無効化
...
リード文はPHPに直書きしているので、固定ページ編集画面にはショートコード1行だけを貼れば完成です。
...
CF7 の管理画面「メール」タブには **メール** と **メール (2)** の2種類があります。

| | 用途 | 宛先 |
|---|---|---|
| **メール** | 管理者へのお問い合わせ通知 | サイト運営者のメールアドレス |
| **メール (2)** | お客様への自動返信メール | フォームに入力されたメールアドレス |

#### メール（管理者宛て通知）

CF7 のデフォルトで有効になっています。以下の通りに設定します。

| 項目 | 設定値 |
|---|---|
| 宛先 | `[_site_admin_email]`（WordPress の管理者メールアドレスを自動取得） |
| 送信元 | `WP COFFEE <wordpress@yourdomain.com>` |
| 件名 | `【WP COFFEE】お問い合わせが届きました：[subject]` |
| 追加ヘッダー | `Reply-To: [email]`（返信するとお客様に届く） |

本文：

```
WP COFFEEのウェブサイトからお問い合わせが届きました。
...
「メール (2) を使用」のチェックボックスをONにして有効化します。

| 項目 | 設定値 |
|---|---|
| 宛先 | `[email]`（お客様が入力したメールアドレスに自動送信） |
| 送信元 | `WP COFFEE <wordpress@yourdomain.com>` |
| 件名 | `【WP COFFEE】お問い合わせありがとうございます` |
...
✅ テスト送信して管理者メール・自動返信メールの両方が届くことを確認する
...
**セキュリティ・バックアップ**
- 【必須】Wordfence Security（総合的なセキュリティ対策）
- 【必須】UpdraftPlus（バックアップ）

**SEO**
- 【まちまち】All in One SEO（SEO対策）

**パフォーマンスアップ系**
- コードサイズ圧縮：Autoptimize
- 画像圧縮：CompressX
- キャッシュによる高速化：WP Super Cache
