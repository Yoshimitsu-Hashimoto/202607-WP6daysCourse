...
<nav class="cat-tabs">
  <!-- ① ALL タブ：絞り込みなし（get_query_var が空）のときだけ is-active を付ける -->
  ...
  <?php
  /* ② 登録済みカテゴリーを get_terms() で取得してタブを動的に生成 */
  $cats = get_terms( [ 'taxonomy' => 'menu_category', 'hide_empty' => true ] );
  foreach ( $cats as $cat ) :
  ?>
  <!-- ③ 現在表示中のカテゴリーに is-active を付ける -->
...
<main>
  <?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
    ...
    <section class="page-content">
      <div class="container">
        <!-- ② 直書きしていたHTMLコンテンツをすべて削除 → the_content() 1行に置き換え -->
        <?php the_content(); ?>
      </div>
    </section>
...
<main>
  ...
  <section class="page-content">
    <div class="container" style="max-width:640px;">
      <?php if ( have_posts() ) : while ( have_posts() ) : the_post(); ?>
        <!-- 静的な HTML フォーム → the_content()（ショートコード経由で CF7 フォームが展開） -->
        <?php the_content(); ?>
      <?php endwhile; endif; ?>
    </div>
  </section>
