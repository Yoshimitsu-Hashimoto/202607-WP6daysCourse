...
<section class="block-news">
  <div class="container">
    ...
    <p class="more-link"><a href="<?php echo esc_url( get_permalink( get_option( 'page_for_posts' ) ) ); ?>">すべて見る</a></p>
  </div>
</section>
```
...
✅ トップページの「すべて見る」リンクが `/news/` に飛ぶ（front-page.php の `get_option('page_for_posts')` を確認）

---

<a id="ch6"></a>
## Chapter 6 | カスタム投稿タイプで「メニュー」を管理しよう
  ...
  <section class="menu-single">
    <div class="menu-single__inner">
      ...
      <div class="menu-single__info">
        ...
        <p class="menu-single__desc"><?php the_excerpt(); ?></p>
      </div>
...
<section class="block-menu">
  <div class="container">
    ...
    <div class="cta-wrap">
      <a class="btn btn--dark" href="<?php echo esc_url( get_post_type_archive_link( 'menu' ) ); ?>">メニューをすべて見る</a>
    </div>
  </div>
</section>
```
...
✅ メニュー編集画面に「価格」「産地」などの入力欄が表示されている
...
<div class="menu-single__info">
  ...
  <?php if ( $price ) : ?>
  <p class="menu-single__price">¥ <?php echo esc_html( number_format( $price ) ); ?> <span>（税込）</span></p>
  <?php endif; ?>

  <?php if ( $desc ) : ?>
  <p class="menu-single__desc"><?php echo nl2br( esc_html( $desc ) ); ?></p>
  <?php endif; ?>

  <table class="spec-table">
    <tbody>
      <?php if ( $origin ) : ?><tr><th>産地</th><td><?php echo esc_html( $origin ); ?></td></tr><?php endif; ?>
      <?php if ( $roast )  : ?><tr><th>焙煎度</th><td><?php echo esc_html( $roast ); ?></td></tr><?php endif; ?>
    </tbody>
  </table>
</div>
```

✅ メニュー詳細ページに価格・産地・焙煎度が表示されている
...
`archive-menu.php` のループ内、`<li class="menu-card">` から `</li>` までの部分を以下に書き換えます。カードに価格を追加し、説明文をカスタムフィールド `menu_description` から取得するように変更します。

```php
<?php if ( have_posts() ) : while ( have_posts() ) : the_post();
  $price = function_exists( 'get_field' ) ? get_field( 'price' )            : '';
  $desc  = function_exists( 'get_field' ) ? get_field( 'menu_description' ) : get_the_excerpt();
?>
  <li class="menu-card">
    <a class="menu-card__link" href="<?php the_permalink(); ?>">
      <figure class="menu-card__fig">
      ...
      <div class="menu-card__body">
        ...
        <?php if ( $price ) : ?><p class="menu-card__price">¥ <?php echo esc_html( number_format( $price ) ); ?></p><?php endif; ?>
      </div>
    </a>
  </li>
<?php endwhile; endif; ?>
...
`function_exists('get_field')` は ACF が有効でない環境でもエラーにならないための書き方。  
一覧ページは複数ファイルで再利用することが多いので、防御的に書くのがよい。
...
```php
<nav class="cat-tabs">
  <a class="cat-tab <?php if ( ! get_query_var( 'menu_category' ) ) echo 'is-active'; ?>"
     href="<?php echo esc_url( get_post_type_archive_link( 'menu' ) ); ?>">ALL</a>
  <?php
  $cats = get_terms( [ 'taxonomy' => 'menu_category', 'hide_empty' => true ] );
  foreach ( $cats as $cat ) :
  ?>
  <a class="cat-tab <?php if ( is_tax( 'menu_category', $cat->slug ) ) echo 'is-active'; ?>"
...
| `'hide_empty' => true` | 記事が0件のカテゴリーは取得しない | 記事のないカテゴリーのタブを表示しないようにするため |
| `get_query_var( 'menu_category' )` | 現在のURLがカテゴリー絞り込み中かどうかを取得する | 「ALL」タブにアクティブクラスを付けるタイミングを判定するため |
| `is_tax( 'menu_category', $cat->slug )` | 表示中のページがそのカテゴリーのアーカイブかを判定する | 現在選択中のカテゴリータブにアクティブクラスを付けるため |
| `get_term_link( $cat )` | そのカテゴリーのアーカイブページURLを返す | 各タブのリンク先URLを正しく生成するため |

「ALL」タブは `get_query_var( 'menu_category' )` が空（絞り込みなし）のときだけ `is-active` クラスが付きます。各カテゴリータブは `is_tax()` で現在のページと一致するタブに `is-active` が付くようになっています。

✅ メニュー一覧ページに「ALL / ドリンク / フード / 豆の販売」タブが表示されている  
...
### 9-1. page.php をブロックエディター対応に書き換える

今の `page.php` にはHTMLが直書きされています。  
コンテンツ部分を `the_content()` 1行に置き換えます。
...
### 9-2. ブロックエディターでコンテンツを入力する

管理画面 → 固定ページ → 「こだわり」を編集。

以下のブロック構成でコンテンツを再現します：

| ブロック | 使い方 |
|---|---|
| 見出し（H2） | 各セクションのタイトル |
| メディアとテキスト | 画像＋テキストの左右レイアウト（左右交互に使う） |
| カラム（3列） | 「3つの想い」— 透明性・持続可能性・静かな体験 |
| テーブル | 「取り扱い産地」— 産地・精製方法・風味・おすすめの飲み方 |
| グループ（背景色付き） | まとめ・実績など |
...
```json
{
  ...
  "settings": {
    "color": {
      ...
      "palette": [
        { "slug": "primary", "color": "#1c2b24", "name": "ダーク" },
        { "slug": "white",   "color": "#ffffff", "name": "ホワイト" },
        { "slug": "cream",   "color": "#f2ede3", "name": "クリーム" },
        { "slug": "accent",  "color": "#b8895a", "name": "アクセント" },
        { "slug": "text",    "color": "#3a3530", "name": "テキスト" },
        { "slug": "muted",   "color": "#8a857d", "name": "グレー" }
      ]
    },
    "typography": {
...
| キー | 値 | 意味 |
|---|---|---|
| `"custom"` | `false` | 任意の色をカラーピッカーで入力できなくする。`palette` で定義したブランドカラーだけ選べる状態になる |
| `"customGradient"` | `false` | 任意のグラデーションの入力を禁止する |
| `"defaultPalette"` | `false` | WordPress が最初から用意しているデフォルトカラー（青・赤・黄など）を非表示にする |
| `"defaultGradients"` | `false` | WordPress デフォルトのグラデーションを非表示にする |
| `"defaultDuotone"` | `false` | WordPress デフォルトの二色調フィルターを非表示にする |
| `"palette"` | 配列 | 編集画面に表示するカラーパレットを定義する。`slug` がCSS変数名のベースになる |

`palette` に色を登録すると、WordPress が `--wp--preset--color--{slug}` という名前のCSS変数を自動生成します。たとえば `"slug": "primary"` と定義すると `--wp--preset--color--primary` が使えるようになります。

`palette` の色の値は、静的サイトの `css/style.css` の `:root` に定義していた変数をそのまま持ってきています。

| slug | 色コード | 元のCSS変数 |
|---|---|---|
| `primary` | `#1c2b24` | `--c-dark` |
| `white` | `#ffffff` | `--c-white` |
| `cream` | `#f2ede3` | `--c-cream` |
| `accent` | `#b8895a` | `--c-accent` |
| `text` | `#3a3530` | `--c-text` |
| `muted` | `#8a857d` | `--c-muted` |

**`typography` セクション**

| キー | 値 | 意味 |
|---|---|---|
| `"defaultFontSizes"` | `false` | WordPress デフォルトのフォントサイズ（Small・Medium・Largeなど）を非表示にする |
| `"fontSizes"` | 配列 | 編集画面の「フォントサイズ」に表示するサイズ選択肢を定義する |

`fontSizes` の値（`0.875rem` / `1rem` / `1.25rem` / `1.75rem`）は静的サイトの CSS で使われているフォントサイズの範囲に合わせた目安値です。ブロックエディターで使う場面は限られるため、粗めの4段階に絞っています。

**`layout` セクション**

静的サイトの `:root` では `--w-content: 900px` / `--w-wide: 1200px` と定義していました。theme.json の `contentSize` と `wideSize` はブロックエディターが管理する幅なので、実際のデザインより少し小さめに設定しています（コンセプトページは本文が中心なので `800px` 、画像などを含む幅広レイアウトは `1100px`）。

**`border` / `spacing` セクション**

枠線や余白のカスタム入力をすべて `false` にして非表示にしています。クライアントが編集画面で余白や枠線を自由に変えられないようにするためです。

✅ ブロックエディターを開くと色の選択肢がブランドカラーのみになっている  
✅ 「納品後にクライアントが勝手に色を変えられない」状態になっている

### 9-4. css/style.css の :root を theme.json の変数に接続する

theme.json を追加すると、WordPressが色・フォントサイズをCSS変数として自動生成します。  
`css/style.css` の `:root` をその変数を参照する形に書き換えます。  
**変えるのは `:root` の中身だけ。それ以外のCSSは一切触りません。**

変更前（`css/style.css` の `:root`）：
...
```css
:root {
  --c-dark:   var(--wp--preset--color--primary);
...
これで theme.json の色を変えるだけでサイト全体のCSSに反映されます。

### 9-5. ブロックエディター用のスタイルを追加する

`the_content()` が出力するブロックHTMLにはWordPress独自のクラスが付きます。  
ハードコーディング時のクラスとは異なるため、`css/style.css` の末尾に追記します。

```css
/* =============================================
   ブロックエディター用スタイル（page.php 内）
   ↓ css/style.css の末尾にまるごと追記する
============================================= */

/* 追記：見出しブロック（H2/H3）のスタイル
   ブロックエディターの見出しには .wp-block-heading が付く。
   静的サイトでは直接 h2 にスタイルを当てていたが、
   ブロック出力では .page-content 内に限定して上書きする。 */
...
.page-content .wp-block-heading:first-child {
  margin-top: 0; /* 追記：先頭見出しの上余白をリセット */
}

/* 追記：「メディアとテキスト」ブロックのスタイル
   画像＋テキストの左右レイアウトに使うブロック。
   gap と縦位置（align-items）を調整して静的サイトのデザインに近づける。 */
...
/* 追記：「テーブル」ブロックのスタイル
   「取り扱い産地」表に使う。
   静的サイトの .hour-table に近いスタイルをブロック向けに書き直している。 */
...
.page-content .wp-block-table thead th {
  background: var(--c-cream); /* 追記：ヘッダー行にクリーム色の背景を追加 */
}

/* 追記：「グループ」ブロックのスタイル
   背景色付きのまとめセクションに使う。
   背景色は編集画面のカラーパレットで指定するため CSS では余白だけ管理する。 */
...
`register_block_style()` を使うと、エディターのサイドパネルに独自のスタイル選択肢を追加できます。選んだスタイルは `is-style-{name}` クラスとして HTML に付与されるだけなので、インラインスタイルは入りません。

こだわりページでは以下のスタイルを登録します。

`functions.php` に追記します。

```php
function wpcoffee_register_block_styles() {

  // グループ：背景色バリエーション
  register_block_style( 'core/group', [ 'name' => 'bg-cream',      'label' => 'クリーム背景' ] );
  register_block_style( 'core/group', [ 'name' => 'bg-cream-dark', 'label' => 'クリーム（濃）背景' ] );
  register_block_style( 'core/group', [ 'name' => 'bg-dark',       'label' => 'ダーク背景' ] );

  // カラム：バリューカード（3つの想い用）
  register_block_style( 'core/column', [ 'name' => 'value-card',    'label' => 'バリューカード' ] );

  // カラム：生産者カード
  register_block_style( 'core/column', [ 'name' => 'producer-card', 'label' => '生産者カード' ] );

  // テーブル：産地テーブル
  register_block_style( 'core/table', [ 'name' => 'origin-table', 'label' => '産地テーブル' ] );
}
add_action( 'init', 'wpcoffee_register_block_styles' );
```

`css/style.css` に対応するスタイルを追記します（グループの背景・余白、カラムのカードレイアウト、テーブルのスタイル）。

✅ エディターでグループブロックを選択すると「スタイル」に背景色の選択肢が表示される  
✅ カラムブロックに「バリューカード」「生産者カード」が選択できる

### 9-7. ブロックパターンをエディターで作る

ブロックパターンは、よく使うブロックの組み合わせをまとめて挿入できる仕組みです。コードを書かずにエディターの UI だけで作成・保存できます。

**作成手順**

1. 管理画面 → **外観 → エディター → パターン** を開く
2. 「新しいパターンを追加」をクリック
3. ブロックエディターで生産者カード1枚分のブロックを組む：
   - カラムブロック（スタイル：生産者カード）を追加
   - 中に画像ブロック＋段落ブロック（名前・農園・説明）を追加
4. タイトルを「生産者カード」にして保存

**挿入方法**

固定ページのエディターで「+」→「パターン」タブ → 「生産者カード」を選んで挿入するだけで、ブロック構成がまるごと展開されます。

✅ パターンを挿入するとブロックがまとめて展開される  
✅ 3カラムに3回挿入して生産者紹介セクションが完成する

### 役割の整理

| | 役割 |
|---|---|
| **ブロックエディター** | コンテンツの構造・配置を決める |
| **css/style.css** | 見た目（色・フォント・余白）を管理する |
| **theme.json** | ブロックエディターのルールを定義する |
| **register_block_style()** | ブロックに選べるスタイルバリエーションを追加する |
| **エディターのパターン機能** | よく使うブロック構成を保存・再利用する |
...
`the_content()` が固定ページ本文（= ショートコード）を出力し、CF7のフォームが展開されます。
