...
`the_content()` を `<main>` の直下に置き、`.container` などで囲まないのがポイントです。こうすることで、ブロックエディターで「全幅」を指定したグループ（9-7 や OUR VALUES パターンで使う）が画面いっぱいに広がります。余白や幅はブロック側（theme.json の `contentSize` / `wideSize`）で管理します。
...
**ポイント：**
- 色・フォント・サイド余白は theme.json の `slug` に対応する変数を参照する（定義元を theme.json に一本化）
- theme.json に無い値（`--c-muted` など）は従来どおり直値で残す
...
function wpcoffee_block_setup() {
  ...
  // セクションタイトル（英語大見出し＋日本語小見出しのまとまり）
  register_block_style( 'core/group',     [ 'name' => 'section-head',    'label' => 'セクションタイトル' ] );
  // 日本語小見出し
  register_block_style( 'core/paragraph', [ 'name' => 'section-subhead', 'label' => 'セクション小見出し' ] );
  // 産地テーブル
...
ここで登録しているのは **section-head / section-subhead / origin-table の3つだけ**です。バリューカード関連（後述のパターンで使う `value-card` など）を登録していないのには理由があります。

| 種類 | 登録 | 理由 |
|---|---|---|
| エディターのパネルから**手で選ぶ**スタイル | **する** | 選択肢として出す必要がある |
| **パターンの中に直書き**されるクラス | **しない** | クラスは HTML に書いてあるので CSS は効く。選択肢に出す必要がない |

### 9-5. ブロックスタイル用の CSS を追加する
...
/* セクションタイトル（is-style-section-head を持つグループ）*/
...
/* 日本語小見出し */
.is-style-section-subhead { font-size: 0.72rem; letter-spacing: 0.12em; color: var(--c-muted); }
...
| クラス | 由来 | CSS の役割 |
|---|---|---|
| `.is-style-section-head` | 登録（手で選ぶ） | セクションタイトルのレイアウト |
| `.is-style-section-subhead` | 登録（手で選ぶ） | 日本語小見出しの文字スタイル |
| `.is-style-origin-table` | 登録（手で選ぶ） | 産地テーブルのデザイン |
| `.is-style-value-card` ほか | パターン直書き | バリューカードの背景・余白・カード番号など |

**登録の有無に関わらず、CSS は対応するクラスがあれば効く**——ここが理解のカギです。
